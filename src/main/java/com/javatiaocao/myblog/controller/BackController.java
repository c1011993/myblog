 ...
 * @version 1.0
 */
@Api(tags = "跳转页面相关模块",value = "包含几块内容")
@Controller
//没有加@ResponseBody, 会把return的字符串解析为视图地址，跳转到对应的页面
//加了这个注解就会把return的字符串当作数据返回
public class BackController {
    ...
    public String index(HttpServletRequest request, HttpServletResponse response) {
        //在不同服务器上获取数据，跨域。*表示可以允许所有域名访问
        response.setHeader("Access-Control-Allow-Origin", "*");
        //判断是否有需要跳回的url，有就将需要跳回的url保存在响应头中
